    ...
    class Reader
      ...
      def read_docker_section section
        DockerSettings.new.tap do |d|
          d.build_image = section['build_image']
          d.build_docker_file = section['build_docker_file']
          d.application_docker_file = section['application_docker_file']
          d.application_compose_file = section['application_compose_file']
        end
      end

      def read_task_section section
        Task.new.tap do |t|
          t.pre = read_pre_section section['pre']
          t.post = read_pre_section section['post']
          t.docker = read_docker_section section['docker']
        end
      end
